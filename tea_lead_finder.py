...
⚠️ Примечания и ответственность:
- Скрипт предназначен для ознакомительного демо. Соблюдайте условия использования OLX и местные законы.
...
- Telegram-бот на pyTelegramBotAPI: команды /start, /scan, /status, /help.
- Режим фонового мониторинга: периодический скан и автоотправка новым подписчикам.

Запуск:
  export BOT_TOKEN="<твой токен от @BotFather>"
  python3 tea_lead_finder.py            # запускает бота + фоновые сканы каждые 6 часов

Быстрый тест без бота:
  python3 tea_lead_finder.py --oneshot   # спарсит и выведет в консоль (без Telegram)
...
Автор: специально для @InetProdvizh
"""
...
SCAN_INTERVAL_MIN = int(os.getenv("SCAN_INTERVAL_MIN", "360"))  # каждые 6 часов по умолчанию
...
HEADERS_POOL = [
    # Несколько юзер-агентов для простейшей ротации
    {
...
def save_leads(conn, leads: List[Dict]) -> List[Dict]:
    """Сохраняет новые лиды, возвращает только те, которые новые."""
    new_items = []
    cur = conn.cursor()
    for lead in leads:
        try:
...
def parse_olx_list(html: str, keyword: str) -> List[Dict]:
    ...
    # OLX часто меняет классы. Берём универсально: ссылки на карточки объявлений.
    for a in soup.find_all("a", href=True):
        href = a["href"]
        if not href:
            continue
        # Фильтруем только ссылки на объявления
        if "/d/" in href and ("/obyavlenie/" in href or "/ogoloshennya/" in href):
            ...
            # Пробуем подняться на несколько уровней для контента
            for _ in range(3):
                if card is None:
                    break
                # Заголовок
    ...
    # Убираем возможные дубликаты по url/id
    unique = {}
    for it in items:
